...
func (n *GCSFuseCSITestDriver) CreateVolume(ctx context.Context, config *storageframework.PerTestConfig, volType storageframework.TestVolType) storageframework.TestVolume {
	...
	case storageframework.PreprovisionedPV:
		...
		case ImplicitDirsVolumePrefix:
			n.CreateImplicitDirInBucket(ctx, ImplicitDirsPath, bucketName, n.EnableZB)
			mountOptions += ",implicit-dirs"
		case SubfolderInBucketPrefix:
			dirPath := uuid.NewString()
			n.CreateImplicitDirInBucket(ctx, dirPath, bucketName, n.EnableZB)
...
func (n *GCSFuseCSITestDriver) CreateImplicitDirInBucket(ctx context.Context, dirPath, bucketName string, zbEnabled bool) {
	storageService, err := n.prepareStorageService(ctx)
	if err != nil {
	...
	if err := storageService.UploadGcsObject(ctx, bucketName, bucketName, fmt.Sprintf("%s/", dirPath), zbEnabled); err != nil {
