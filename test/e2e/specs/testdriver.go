...
const (
	gcsfusecsiProfilesControllerTestBucket       = "gcsfusecsi-profiles-controller-test-bucket"
	gcsfusecsiProfilesControllerTestBucketRegion = "us-central1"
...
func (n *GCSFuseCSITestDriver) CreateVolume(ctx context.Context, config *storageframework.PerTestConfig, volType storageframework.TestVolType) storageframework.TestVolume {
	...
	case storageframework.PreprovisionedPV:
		...
		case ProfilesControllerTestPrefix:
			bucketName = gcsfusecsiProfilesControllerTestBucket
			// IAM policy is handled by the test drivers create bucket flow,
			...
			serviceBucketobj := &storage.ServiceBucket{
				...
				Name:                           bucketName,
				Location:                       gcsfusecsiProfilesControllerTestBucketRegion,
