...
func (n *GCSFuseCSITestDriver) CreateVolume(ctx context.Context, config *storageframework.PerTestConfig, volType storageframework.TestVolType) storageframework.TestVolume {
	...
	case storageframework.PreprovisionedPV:
		...
		case ProfilesOverrideAllOverridablePrefix:
			...
			// Create buckets for Profiles Override All Overidable test
			bucketName = n.createBucket(ctx, config.Framework.Namespace.Name)
			n.giveDriverAccessToVolumes(ctx, bucketName, projectNumber)
...
func (n *GCSFuseCSITestDriver) giveDriverAccessToVolumes(ctx context.Context, bucket_name string, project_number string) error {
	...
	member := fmt.Sprintf("principal://iam.googleapis.com/projects/%s/locations/global/workloadIdentityPools/%s.svc.id.goog/subject/ns/gcs-fuse-csi-driver/sa/gcs-fuse-csi-controller-sa", project_number, n.meta.GetProjectID())
