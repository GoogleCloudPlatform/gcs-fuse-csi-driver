...
// overrides the `gke-gcefuse-cache` with dummy values, needed for certain workflows in profiles.
...
func (t *TestPod) VerifyMountOptionsArePassed(namespace string, mountOptions map[string]string) {
	// 1. Get logs
	stdout, stderr, err := runKubectlWithFullOutputWithRetry(namespace, "logs", t.pod.Name, "-c", "gke-gcsfuse-sidecar")
	framework.ExpectNoError(err,
		"Error accessing logs from pod %v, but failed with error message %q\nstdout: %s\nstderr: %s",
	...
	for key, value := range mountOptions {
		expectedFlagString := fmt.Sprintf(`%s:%s`, key, value)

		gomega.Expect(stdout).To(gomega.ContainSubstring(expectedFlagString),
			"Should find flag string %q in stdout", expectedFlagString)
	}
}
...
func (t *TestPod) getDriverLogs() (string, string, error) {
	...
	// Isolate controller pod on test pod node.
