...
func getBoskosProject(resourceType string) *common.Resource {
	...
	for {
		...
		case <-ticker.C:
			...
			// Fails with: "Post \"http://boskos/acquire?dest=busy&owner=&state=free&type=gke-internal-project\": dial tcp: lookup boskos on 127.0.0.1:53: no such host"
			if err != nil {
				klog.Warningf("boskos failed to acquire project: %q", err.Error())
			} else if p == nil {
