...
func (t *gcsFuseCSIGCSFuseIntegrationTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	gcsfuseIntegrationTestNew := func(testPkg string, testName string, config utils.ParsedConfig, secondaryConfig *utils.ParsedConfig) {
		...
		// Run readonly test cases requiring file-mode/dir-mode tests as a non-root user (uid/gid 6666).
		// This ensures the kernel correctly enforces "Permission denied" instead of bypassing it as root.
		if testPkg == testNameReadonly && !config.ReadOnly {
			config.MountOptions = append(config.MountOptions, "uid=6666", "gid=6666")
		}
	...
	generateDynamicTests := func(configVersion string) {
		...
		for pkgName, pkgList := range utils.LoadedTestPackages {
			//TODO: delete after b/496956438 got fixed
			if pkgName == testNameBenchmarking {
				continue
			}
			//TODO: delete after b/496966414 got fixed
