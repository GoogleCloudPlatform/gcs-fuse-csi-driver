...
func (t *gcsFuseCSIGCSFuseIntegrationTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	gcsfuseIntegrationTest := func(testName string, readOnly bool, mountOptions ...string) {
		...
		baseTestCommand := fmt.Sprintf("export GOTOOLCHAIN=go1.22.2 && export PATH=$PATH:/usr/local/go/bin && cd %v/%v && GODEBUG=asyncpreemptoff=1 go test . -p 1 --integrationTest -v --mountedDirectory=%v", gcsfuseIntegrationTestsBasePath, testName, mountPath)
