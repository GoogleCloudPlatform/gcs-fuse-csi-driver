...
func (t *gcsFuseCSIProfilesTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	init := func(configPrefix ...string) *storageframework.PerTestConfig {
		...
		switch testScenario {
		case specs.ProfilesControllerTestPrefix:
	...
	ginkgo.It("should rescan pv after controller is killed", ginkgo.Serial, func(ctx context.Context) {
		testConfig := init(specs.ProfilesControllerTestPrefix)
		killController(ctx, testConfig)
		defer cleanup()

		// Bucket scan on 20M objects takes 1 minute and 30 seconds, waiting for controller to come up and complete scan.
		time.Sleep(time.Minute * 3)

		// Verify bucket was scanned and save the value for later.
		validatePVAnnotations(l.volumeResourceList[0].Pv.Name, map[string]string{
			putil.AnnotationLastUpdatedTime: "anything", // This value is ignored, we just verify a timestamp was set
		}, ctx)

		pv := getPV(l.volumeResourceList[0].Pv.Name, ctx)
		lastScanTime, exists := pv.Annotations[putil.AnnotationLastUpdatedTime]
		gomega.Expect(exists).To(gomega.BeTrue(), fmt.Sprintf("annotation %s not found on PV %s", putil.AnnotationLastUpdatedTime, pv.Name))
		...
		pv = getPV(l.volumeResourceList[0].Pv.Name, ctx)
		newScanTime, exists := pv.Annotations[putil.AnnotationLastUpdatedTime]
		gomega.Expect(exists).To(gomega.BeTrue(), fmt.Sprintf("annotation %s not found on PV %s", putil.AnnotationLastUpdatedTime, pv.Name))
		gomega.Expect(newScanTime).NotTo(gomega.BeEmpty())
		gomega.Expect(newScanTime).NotTo(gomega.Equal(lastScanTime))
	...
	ginkgo.It("should override profiles all overridable values", ginkgo.Serial, func() {
		...
		if IsOSSEnvVar == "true" {
			ginkgo.Skip("Skipping test: this feature is only supported on Linux")
		}
