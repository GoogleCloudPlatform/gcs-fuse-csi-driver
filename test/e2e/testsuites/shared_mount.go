...
func (t *gcsFuseCSISharedMountTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	scrapePrometheusMetrics := func(tPod *specs.TestPod, vr *storageframework.VolumeResource, mountPath string) (map[string]*dto.MetricFamily, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to create temp dir for metrics: %w", err)
		}
		promLocalPath := filepath.Join(promLocalDir, promFileName)
		defer os.Remove(promLocalPath)

		return scrapeGCSFuseMetrics(ctx, f, gcsfuseDriver, tPod, vr.Pv.Spec.CSI.VolumeHandle, mountPath, promFileName, promLocalPath)
	}
	...
	verifyVolumeMetrics := func(g gomega.Gomega, families map[string]*dto.MetricFamily, bucketName, volumeName, expectedMounterPodName, workloadPodName, namespace string) {
		...
		for _, metricName := range expectedMetricNames {
			...
			metricsList := metricsForVolume(mf, bucketName, expectedMounterPodName, volumeName, namespace, true /* expectEmptyPodUID */)
		...
		fsOpsMetrics := metricsForVolume(fsOpsFamily, bucketName, expectedMounterPodName, volumeName, namespace, true /* expectEmptyPodUID */)
	...
	ginkgo.It("[shared-mount] metrics should emit metrics for a single volume with mounter pod name label", func() {
		...
		gomega.Eventually(ctx, func(g gomega.Gomega) {
			families, err := scrapePrometheusMetrics(workloadPod, sharedVR, sharedMountPath)
			g.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("[shared-mount] metrics should emit separate metrics for multiple volumes on a single workload pod", func() {
		...
		gomega.Eventually(ctx, func(g gomega.Gomega) {
			families, err := scrapePrometheusMetrics(workloadPod, vr1, mountPath1)
			g.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("[shared-mount] metrics should collect aggregated metrics under shared mounter pod for multiple workload pods", func() {
		...
		gomega.Eventually(ctx, func(g gomega.Gomega) {
			families, err := scrapePrometheusMetrics(podA, sharedVR, sharedMountPath)
			g.Expect(err).NotTo(gomega.HaveOccurred())
			...
			// verifyVolumeMetrics already asserted fs_ops_count is present for this Mounter
			// Pod, so just total it up to check both pods' operations are aggregated.
			metricsList := metricsForVolume(families["fs_ops_count"], bucketName, mounterPod.Name, volumeName, f.Namespace.Name, true /* expectEmptyPodUID */)

			var totalFsOps uint64
			for _, m := range metricsList {
				totalFsOps += uint64(m.GetCounter().GetValue())
			}
