...
func ParseConfigFlags(flagStr string) ParsedConfig {
	...
	for _, f := range strings.Split(flagStr, ",") {
		...
		if configPrefix, ok := disallowedFlagsMapping[flagName]; ok {
			...
			default:
				// For all other disallowed flags that don't have a specific manual handling case,
				// we swallow them to prevent errors or unauthorized configuration changes.
				continue
			}
		...
		if f == flagFileCacheEnableParallelDownloads || strings.HasPrefix(f, flagFileCacheEnableParallelDownloads+"=") {
			f = "file-cache:enable-parallel-downloads:" + getFlagValue(f)
		} else if f == flagFileCacheEnableODirect || strings.HasPrefix(f, flagFileCacheEnableODirect+"=") {
			f = "file-cache:enable-o-direct:" + getFlagValue(f)
		} else if f == flagEnableKernelReader || strings.HasPrefix(f, flagEnableKernelReader+"=") {
			f = "file-system:enable-kernel-reader:" + getFlagValue(f)
		} else if f == flagFileCacheEnableCrc || strings.HasPrefix(f, flagFileCacheEnableCrc+"=") {
			f = "file-cache:enable-crc:" + getFlagValue(f)
		} else if f == flagFileCacheCacheFileForRangeRead || strings.HasPrefix(f, flagFileCacheCacheFileForRangeRead+"=") {
			f = "file-cache:cache-file-for-range-read:" + getFlagValue(f)
		}
...
// getFlagValue extracts the value from a flag string (e.g. "flag=value").
// It defaults to "true" if the flag is passed without a value or with an empty assignment.
func getFlagValue(f string) string {
	val := "true"
	if _, v, found := strings.Cut(f, "="); found && v != "" {
		val = v
	}
	return val
}
