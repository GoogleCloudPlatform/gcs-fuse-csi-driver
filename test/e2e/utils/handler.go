...
func generateTestSkip(testParams *TestParameters) string {
	...
	if testParams.UseGKEManagedDriver {
		skipTests = append(skipTests, "Pod.RestartPolicy.is.OnFailure$", "Job.with.RestartPolicy.OnFailure.eventually.succeed", "fast.termination", "fileCache", "gcsfuseIntegrationFileCache", "init.container", "istio", "csi-skip-bucket-access-check", "concurrent_operations", "kernel_list_cache", "metrics")
