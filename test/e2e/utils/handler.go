...
func generateTestSkip(testParams *TestParameters) string {
	...
	if testParams.UseGKEManagedDriver {
		skipTests = append(skipTests, "Pod.RestartPolicy.is.OnFailure$", "Job.with.RestartPolicy.OnFailure.eventually.succeed", "fast.termination", "fileCache", "gcsfuseIntegrationFileCache", "init.container", "istio", "csi-skip-bucket-access-check", "concurrent_operations", "kernel_list_cache", "service.account.permission.changes", "local_file")
		...
		if strings.HasPrefix(testParams.GkeClusterVersion, "1.26") || strings.HasPrefix(testParams.GkeClusterVersion, "1.27") || strings.HasPrefix(testParams.GkeClusterVersion, "1.28") {
			skipTests = append(skipTests, "list_large_dir", "concurrent_operations", "kernel_list_cache")
		}
	}
