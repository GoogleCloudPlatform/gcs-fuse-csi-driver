...
func TestParseConfigFlags(t *testing.T) {
	...
	t.Run("Testing ParseConfigFlags", func(t *testing.T) {
		...
		testCases := []struct {
			...
			{
				name:    "should handle empty assignment correctly for all boolean flags",
				flagStr: "--file-cache-enable-parallel-downloads=,--file-cache-enable-o-direct=,--enable-kernel-reader=",
				expectedParsed: ParsedConfig{
					FileCacheCapacity: "50Mi",
					ReadOnly:          false,
					LogSeverity:       "info",
					MountOptions: []string{
						"file-cache:enable-parallel-downloads:true",
						"file-cache:enable-o-direct:true",
						"file-system:enable-kernel-reader:true",
					},
				},
			},
