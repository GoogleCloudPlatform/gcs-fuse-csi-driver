...
func (mgr *fakeNetworkManager) AddRule(table int, sourceIP string) error {
	...
	mgr.rules = append(mgr.rules, Rule{Table: table, Source: sourceIP})
	return nil
}
...
func TestSourceRouteForDevice(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			expectedNewRules:  []Rule{{Source: "10.128.0.13", Table: 50}},
		...
		},
		{
			...
			rules:          []Rule{{Source: "10.128.0.13", Table: 50}},
			expectedSource: "10.128.0.13",
		},
		{
			name:    "other routes",
			...
			expectedNewRules:  []Rule{{Source: "10.128.0.13", Table: 51}},
			expectedSource:    "10.128.0.13",
		},
		{
			name:     "route no rule",
		...
		},
		{
			...
			rules:             []Rule{{Source: "10.128.0.13", Table: 51}},
			expectedNewRoutes: []Route{{Device: "eth0", Gateway: "10.128.0.1", Table: 51}},
			expectedSource:    "10.128.0.13",
		},
		{
