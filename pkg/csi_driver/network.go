...
/*
if gcs fuse table in rt_tables, get id
otherwise scale tables to get a new id, add to rt_tables
ah, but we need one table per source/device. Not big deal as fixed per vm.
create on-demand in case other devices used for something else.

Table will be gcsfusecsi_DEVICE.
*/

// GetDeviceForNumaNode returns a device name for the node, if one exists. If
// there are multiple devices, a gVINC driver is perferred. The message return
// value is a string that can be displayed in an event as a warning to the
// user.
...
// AddSourceRouteForDevice checks for an existing source routing for device, and adds it if necessary. The source route used is returned.
func AddSourceRouteForDevice(mgr NetworkManager, device string) (string, error) {
	...
	for _, rule := range tableRules {
		if rule.Source == source && rule.Table == table {
			foundRule = true
			break
		}
	}
...
func getRtTables(mgr NetworkManager) ([]int, []string, error) {
	...
	for _, line := range strings.Split(tableString, "\n") {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("Bad table id: %s", trimmed)
		}
		ids = append(ids, id)
...
func (_ *realNetworkManager) UpdateRouteTable(line string) (err error) {
...
func (mgr *realNetworkManager) AddRoute(table int, gatewayIP, device string) error {
	link, err := mgr.getDeviceLink(device)
	if err != nil {
		return fmt.Errorf("AddRoute: %w", err)
	}
