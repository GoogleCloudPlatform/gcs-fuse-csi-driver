...
func (s *nodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if mounted {
		...
		// Start GCSFuse Kernel Params monitoring for non dynamic mounts. This invokes a single long lived goroutine
		// which is terminated from NodeUnpublishVolume operation, this scenario fires monitoring
		// go routine for existing mounts when driver restarts.
		if s.driver.config.FeatureOptions.EnableGCSFuseKernelParams && args.bucketName != "_" {
