...
func isSidecarVersionSupportedForTokenServer(imageName string) bool {
	managedSidecarPattern := `.*/gke-release/gcs-fuse-csi-driver-sidecar-mounter:v\d+.\d+.\d+-gke\.\d+.*`
	re := regexp.MustCompile(managedSidecarPattern)
	isManagedSidecar := re.MatchString(imageName)

	if !isManagedSidecar {
		klog.Infof("mountOptions should not be passed because this is a private sidecar %v", imageName)
