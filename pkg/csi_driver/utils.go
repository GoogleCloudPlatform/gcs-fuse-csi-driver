...
const (
	...
	GCSFuseCacheDirInternalFlag            = "cache-dir-internal"
)
...
func (driver *GCSDriver) generateDisallowedFlagsMap(gcsFuseSidecarImage string) (map[string]bool, error) {
	...
	shouldPassProfileFlags := driver.config.FeatureOptions.FeatureGCSFuseProfiles.EnableGcsfuseProfilesInternal && isSidecarVersionSupportedForGivenFeature(gcsFuseSidecarImage, GCSFuseProfilesMinVersion)
	if !shouldPassProfileFlags {
		disallowedFlags[GCSFuseProfileFlag] = true
		// disallowedFlags[GCSFuseCacheDirInternalFlag] = true
	}
