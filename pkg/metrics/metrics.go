...
type Manager interface {
	...
	RegisterMetricsCollector(podName, targetPath, emptyDirBasePath, bucketName string)
...
func (mm *manager) RegisterMetricsCollector(podName, targetPath, emptyDirBasePath, bucketName string) {
	...
	if err := mm.registry.Register(c); err != nil && !strings.Contains(err.Error(), prometheus.AlreadyRegisteredError{}.Error()) {
		klog.Errorf("failed to register metrics collector for pod %q, volume %q, bucket %q: %v", podName, volumeName, bucketName, err)
	}
}
...
func (mm *manager) UnregisterMetricsCollector(targetPath string) {
	...
	if ok := mm.registry.Unregister(c); !ok {
		klog.Errorf("failed to unregister metrics collector for targetPath %q", targetPath)
	}
}
