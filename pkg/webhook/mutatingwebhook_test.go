	...
	testCases := []struct {
		...
		},
		{
			...
			wantConfig: &Config{
				...
				EphemeralStorageLimit:   resource.MustParse("50Gi"),
				EphemeralStorageRequest: resource.Quantity{},
				MetadataPrefetchMemoryLimit: resource.MustParse("20Mi"),
				MetadataPrefetchMemoryRequest: resource.MustParse("20Mi"),
			},
			expectErr: false,
		},
		{
			name: "metadata prefetch limits are provided",
			annotations: map[string]string{
				GcsFuseVolumeEnableAnnotation:     "true",
				cpuLimitAnnotation:                "500m",
				memoryLimitAnnotation:             "1Gi",
		...
		{
			name: "metadata prefetch requests are provided",
			annotations: map[string]string{
				GcsFuseVolumeEnableAnnotation:     "true",
				cpuLimitAnnotation:                "500m",
				memoryLimitAnnotation:             "1Gi",
