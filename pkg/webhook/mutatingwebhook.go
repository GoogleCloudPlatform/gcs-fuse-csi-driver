...
func (si *SidecarInjector) Handle(ctx context.Context, req admission.Request) admission.Response {
	if req.Kind.Kind == "PersistentVolume" {
		...
		if err := si.validatePersistentVolume(pv); err != nil {
...
// validates the PersistentVolume to ensure it meets the requirements for gcsfuse CSI driver usage
// currently validations are only required by storage profiles, so this is a no-op if profiles are not enabled
func (si *SidecarInjector) validatePersistentVolume(pv *corev1.PersistentVolume) error {
	klog.Infof("Validating PersistentVolume %s", pv.Spec.CSI.Driver)
	if pv.Spec.CSI == nil || pv.Spec.CSI.Driver != util.GCSFuseCsiDriverName {
		// Not a gcsfuse CSI driver volume, skip validation
		return nil
	}
	klog.Info("Validating PersistentVolume TWO")
	if si.Config.EnableGcsfuseProfiles { // Currently, only storage profiles require pv validation
		err := putil.ValidateStorageProfilesOverrideStatus(pv)
		if err != nil {
			return err
		}
	}
