...
func TestCreateAnywhereCache(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupMocks: func() {
				utilGetZonesForClusterLocation = func(ctx context.Context, service *compute.Service, location string) ([]string, error) {
		...
		},
		{
			...
			setupMocks: func() {
				utilGetZonesForClusterLocation = func(_ context.Context, service *compute.Service, location string) ([]string, error) {
					return []string{}, nil
				}
			},
			storageControlClient: &mockStorageControlClient{},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.setupMocks()
			s := &Scanner{eventRecorder: fakeRecorder, config: &ScannerConfig{ClusterLocation: "us-central1"}, storageControlClient: tc.storageControlClient}
