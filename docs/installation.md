...
**Important**: Use the same version (e.g., `v1.21.5`) across all components, as version skew is not extensively tested. While these values are set by default, explicitly setting them ensures compatibility with the rest of this guide for both public and custom image installations.
