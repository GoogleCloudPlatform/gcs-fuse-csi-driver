...
# Mine is 34.67.73.212, so I replaced that. 
...
# Mine is:
# kubeadm join 34.67.73.212:6443 --token gxz77g.btow7pvyrs36t4l0 \
#         --discovery-token-ca-cert-hash sha256:b952356c996fb4da81969830131b6c63b351136d3410225de54464b01ee53bce
...
# find SystemdCgroupin /etc/containerd/config.toml and update to true
...
[Workload Identity Federation for GKE](https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity#authenticating_to) is not supported for the GCSFuse CSI driver on OSS K8s. Instead, you must [link your KSA to IAM](https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity#kubernetes-sa-to-iam) with the following steps. Linking a Kubernetes Service Account with a GCP SA (an alternative to Workload Identity Federation for GKE) is required so the driver can configure the KSA to impersonate IAM service accounts, which configures GKE to exchange the federated access token for an access token from the IAM Service Account Credentials API.


```bash
# Set GSA to your own value.
export GSA=amacaskill-gcs-fuse
...
# move cluster-jwks.json from the master vm the dev machine in order to run the below gcloud commands.
...
## Create resources for Workload Identity

Create a Kubernetes ServiceAccount for your application to use. You can also use any existing
Kubernetes ServiceAccount in any namespace, including the default Kubernetes ServiceAccount.

```bash
# IMPORTANT: Replace namespace with the namespace you want to use.
# Export outside VM for use in gcloud commands later.
export NAMESPACE=gcsfuse-test-ns
gcloud compute ssh kub-m --zone=$ZONE --project=$PROJECT_ID
sudo su -
...
kubectl create serviceaccount k8s-on-gce-ksa --namespace $NAMESPACE
...
kubectl annotate serviceaccount k8s-on-gce-ksa \
    --namespace $NAMESPACE \
...
--member="principal://iam.googleapis.com/projects/$PROJECT_NUMBER/locations/global/workloadIdentityPools/$WORKLOAD_IDENTITY_POOL/subject/system:serviceaccount:$NAMESPACE:k8s-on-gce-ksa"
...
# copy contents of credential-configuration.json to master VM
...
# IMPORTANT, replace value of SUDO_USER below value you copied above when sshed with sudo -s above.
export S_USER=SUDO_USER
# IMPORTANT: Replace namespace with the namespace you want to use.
export NAMESPACE=gcsfuse-test-ns
...
### Test Workload Identity (Optional)
...
export SA_TOKEN_AUDIENCE="//iam.googleapis.com/projects/$PROJECT_NUMBER/locations/global/workloadIdentityPools/$WORKLOAD_IDENTITY_POOL/providers/$WORKLOAD_IDENTITY_POOL_PROVIDER"
echo "SA_TOKEN_AUDIENCE = $SA_TOKEN_AUDIENCE"
...
metadata:
 name: example
 # IMPORTANT: replace this with the value of NAMESPACE you set in 'Create resources for Workload Identity'
 namespace: <NAMESPACE>
spec:
 containers:
 - name: example
 ...
 serviceAccountName: k8s-on-gce-ksa
 volumes:
 - name: token
   projected:
     sources:
...
export IDENTITY_PROVIDER="//iam.googleapis.com/projects/$PROJECT_NUMBER/locations/global/workloadIdentityPools/$WORKLOAD_IDENTITY_POOL/providers/$WORKLOAD_IDENTITY_POOL_PROVIDER"
# NOTE: You created this in create private pool section
export WORKER_POOL_NAME=amacaskill-worker
export WORKER_POOL=projects/$PROJECT_ID/locations/$REGION/workerPools/$WORKER_POOL_NAME

# Submit the Cloud Build job with the configured values
gcloud builds submit . --config=cloudbuild-install.yaml --worker-pool=$WORKER_POOL --region=$REGION --substitutions=_REGISTRY=$REGISTRY,_PROJECT_ID=$PROJECT_ID,_IDENTITY_POOL=$IDENTITY_POOL,_IDENTITY_PROVIDER=$IDENTITY_PROVIDER,_SELF_MANAGED_K8S=true,_KUBECONFIG_SECRET=$KUBECONFIG_SECRET,_STAGINGVERSION=custom-$LATEST_TAG
```
...
1. **Create a Cloud Storage Bucket** following instructions [here](https://cloud.google.com/storage/docs/creating-buckets).

2. **Deploy the pod.**

Deploy the pod below:
...
spec:
  ...
  serviceAccountName: k8s-on-gce-ksa # Make sure this is bound to your GSA
  containers:
  - image: busybox
    name: busybox
